...
(function ($, _, undefined) {
    ...
    var utils = {
        ...
        highlightTerm: function (value, term) {
            ...
            term = term.replace(new RegExp('[ÁÀÂÃÄ]','gi'), 'A');
            term = term.replace(new RegExp('[ÉÈÊË]','gi'), 'E');
            term = term.replace(new RegExp('[ÍÌÎÏ]','gi'), 'I');
            term = term.replace(new RegExp('[ÓÒÔÕÖ]','gi'), 'O');
            term = term.replace(new RegExp('[ÚÙÛÜ]','gi'), 'U');
    ...
    var TextNTags = function (editor) {
        ...
        function setSettings (options) {
            ...
            _.each(settings.triggers, function (val, key) {
                ...
                //settings.triggers[key].finder = new RegExp(regex_key + '(\\w+)?(\\s+)?', 'gi');
            });
        ...
        function noElastic() {
            $('.tagged_text_ex4, textarea', elmWrapperBox).wrapAll('<div class="container-mention-scroll"/>').wrapAll('<div class="container-mention"/>');
        }
        ...
        function checkForTrigger(look_ahead) {
            ...
            found_trigger = _.find(settings.triggers, function (trigger, tchar) {
                ...
                //var matches = left_text.match(/[#@]?/gi);
                if (matches) {
        ...
        function addTag (tag) {
            ...
            var trigger = settings.triggers[currentTriggerChar],
                ...
                newCaretPosition = tagStart + localTag.title.length + 1,
